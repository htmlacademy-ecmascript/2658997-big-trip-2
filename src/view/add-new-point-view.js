...
function createDestinationOptionTemplate(city) {
  return `
...
function createNewPointTemplate(destinations, offers) {
  const typeItemsTemplate = TRIP_TYPES
    .map((typeItem) => createTypeItemTemplate(typeItem))
    .join('');

  const destinationOptionsTemplate = destinations
    .map((cityItem) => createDestinationOptionTemplate(cityItem))
    .join('');
